		...
		if( !isset( $this->translations[$domain] ) )
		{
			...
			foreach( $locations as $location )
			{
				if( ( $content = file_get_contents( $location ) ) === false ) {
					throw new \Aimeos\Base\Translation\Exception( 'No translation file "%1$s" available', $location );
				}

				if( ( $content = unserialize( $content ) ) === false ) {
					throw new \Aimeos\Base\Translation\Exception( 'Invalid content in translation file "%1$s"', $location );
				}
